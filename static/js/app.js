...
function buildCharts(id,data) {
   ...
   // Build a Bubble Chart
    var traceBubble = {
      ...
      mode: 'markers',
      marker: {color: sampleData.otu_ids, size: sampleData.sample_values, colorbar: {tickvals: [2,3663]}} // Specific ticks on the color bar
    };
    ...
    let traceBar = {
      ...
      type: "bar",
      orientation: "h",
      marker: {color: barData.map(row => row.otu_ids), colorbar: {tickvals: [2,3663]}}
    };
    // Data array
...
function optionChanged(newSample) {
  json.then( data => {
    ...
    Plotly.restyle("bubble", 'marker', [{color: newData.otu_ids, size: newData.sample_values, colorbar: {tickvals: [2,3663]}}]);
    // Redraw Bar Charts
    ...
    Plotly.restyle("bar", 'marker', [{color: barData.map(row => row.otu_ids), colorbar: {tickvals: [2,3663]}}]);
    //Update metadata
